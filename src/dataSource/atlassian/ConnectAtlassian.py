...
class ConnectAtlassian:
        ...
        while (not end_of_stream) and (retries < 4):
            ...
            if response.status_code == 429:  # rate limit hit
                ...
                if response.json()["issues"] == []:
                    ...
                    start_at = (
                        response.json()["startAt"] + response.json()["maxResults"]
                    )
                    # print(start_at)
                    # print(end_of_stream)
    ...
    def createDataFrame(self, project, jiraService, engine, refresh_IDX):
        ...
        jql = json.load(
            codecs.open(
                "src/dataSource/atlassian/config/JiraAttributes.json", "r", "utf-8"
            )
        )

        if jiraService in ("jira_tsc", "jira_tsc1"):
            jql_fields = (
                ", ".join(jql["general"]["standard"])
                + ", "
                + ", ".join(jql["general"]["customFields"])
            )
            filter = ", ".join(jql["general"]["filter"])
        elif jiraService == "jira_gilds":
            jql_fields = (
                ", ".join(jql["general"]["standardGilds"])
                + ", "
                + ", ".join(jql["general"]["customFieldsGilds"])
            )
            ...
            dfCore = df.loc[
                ...
                df.columns.isin(
                    [
                        ...
                        "fields.duedate",
                        "fields.updated",
                        "fields.customfield_14455.value",
            ...
            dfCore = df.loc[
                ...
                df.columns.isin(
                    [
                        ...
                        "fields.customfield_20118.value",
                        "fields.customfield_19900.value",
                        "fields.customfield_20081.value",
                        "fields.cusotmfield_20003.value",
                        "fields.customfield_20128.value",
                        "fields.customfield_19901.value",
                        "fields.customfield_20080.value",
                        "fields.cusotmfield_20004.value",
                        "fields.customfield_20129.value",
                        "fields.customfield_19890.value",
                    ]
                ),
            ]
            ...
            dfSatelite = df.loc[
                ...
                df.columns.isin(
                    [
                        ...
                        "fields.fixVersions",
                        "fields.components",
                        "fields.customfield_20006.id",
                        "fields.customfield_20006.name",
                        "fields.customfield_20006.archived",
                        "fields.customfield_20006.released",
                        "fields.customfield_20006.releaseDate",
                        "fields.customfield_20141.id",
                        "fields.customfield_20141.name",
                        "fields.customfield_20141.archived",
                        "fields.customfield_20141.released",
                        "fields.customfield_20141.releaseDate",
                        "fields.customfield_14162.id",
                        "fields.customfield_14162.name",
                        "fields.customfield_14162.archived",
                        "fields.customfield_14162.released",
                        "fields.customfield_14162.releaseDate",
                    ]
                ),
            ]
    ...
    def GetIssues(self, project, jiraService, engine, refresh_IDX):
        ...
        for index, row in projects.iterrows():
            issues, issues_copy = self.createDataFrame(
                row["jira_project"], jiraService, engine, refresh_IDX
            )
            ...
            for x in datetimex:
                try:
                    issues[x] = issues.apply(
                        lambda y: self.convertToDateTime(y[x]), axis=1
                    )
                except KeyError:
                    print("Field : " + x + " not found")

            if (
                row["jira_project"] == "SIMS"
                or row["jira_project"] == "GCT"
                or row["jira_project"] == "AEC"
            ):
                issues["defect_age"] = "NaN"
                print(
                    "missing fields: defect_age for project : "
                    + row["jira_project"]
                    + " has been set!"
                )
            if row["jira_project"] == "GXD":
                issues["defect_age"] = "NaN"
                print(
                    "missing fields: defect_age and compontentsname for project : "
                    + row["jira_project"]
                    + " has been set!"
                )
            if row["jira_project"] == "IRE":
                issues["defect_age"] = "NaN"
                print(
                    "missing fields: ALL for project : "
                    + row["jira_project"]
                    + " has been set!"
                )
