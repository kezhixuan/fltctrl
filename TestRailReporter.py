    ...
    with open(file_name) as f:
        ...
        for line in f:
            match_string = re.search(
                r"\{STAR_VERSION_LABEL\}\s=\sStar\s+(\d+\.?)+", line
            )
        ...
        if self.start_test(test) is not False:
            ...
            try:
                ...
                if testcase_id is not None:
                    ...
                    duplicate_test_name = _get_duplicate_test_name(
                        tag_for_separate_section, test.name
                    )
...
def _create_bulk_result_and_logs_for_failed_tests(visitor, outputfile_path):
    ...
    for result in visitor.results:
        if result.test_status == "PASS":
            ...
            fail_message = (
                f"Test case failed with following message:"
                f"{result.message}"
            ...
            rebot(
                outputfile_path,
                outputdir=root_folder,
                include=testcase_id_tag,
                log=testcase_id_tag,
                report=None,
            )
        else:
...
def _add_results(testrun_id, bulk_result):
    ...
    result = client.send_post(url, {"results": bulk_result})
    return result
...
def _generate_message():
    ...
    for test_result in visitor.results:
        ...
        if (
            ...
            and test_result.tag_for_separate_section is None
        ):
            teams_result.increase_fail_counter(1)
            teams_result.append_fail_message_body(test_result)
        elif (
            test_result.test_status == "FAIL"
            and test_result.tag_for_separate_section is not None
        ):
...
if report_to_test_rail == "true":
    ...
    test_run_id = _create_run(
        plan_id,
        stage_name,
        branch_name,
        [item.testcase_id for item in visitor.results],
        star_version,
    )
    ...
    result_ids = [
        [index, result["id"]]
        for index, result in enumerate(test_rail_results)
