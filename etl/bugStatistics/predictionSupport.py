...
class predictQA(connectDB):
    ...
    def getTestRailID(self, project):
        print(project)
        testrail_id = self.config[self.config["jira_project"] == project][
            "testrail_id"
        ].iloc[0]
    ...
    def getFieldCorrelation(self):
        ...
        bugs_stats["testrail_id"] = bugs_stats["Project"].apply(
            lambda x: self.getTestRailID(x)
        )
        ...
        scatterplotmatrix(
            bugs_stats[cols].values, figsize=(10, 8), names=cols, alpha=0.5
        )
        plt.tight_layout()
        plt.show()
    ...
    def getTestRunAggregated(self):
        sql_str = (
            "select DATEDIFF(day, created_on, updated_on) as duration, passed_count, blocked_count, untested_count, failed_count, retest_count,project_id, created_on, updated_on, is_completed "
