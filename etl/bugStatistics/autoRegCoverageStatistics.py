...
class regressionCoverage(connectDB):
    ...
    def getRegressionCases(self):
        with self.engine.connect() as connection:
            ...
            print(lastCycle)
            sql_str = ("select count(tc.id), tc.projectJI, tc.custom_automated, tc.custom_regressiontype  " +
                       " from sq.dim_tr_cases tc where tc.custom_automated in (1) " +
                       " and tc.custom_regressiontype = 1 " +
                       " and tc.Refresh_Cycle = " + lastCycle + "" +
                       " group by tc.projectJI, tc.custom_regressiontype, tc.custom_automated ")
            print(sql_str)
            sql_all = ("select tc.projectJI, tc.custom_automated, tc.custom_regressiontype, tc.id" +
                    " from sq.dim_tr_cases tc " +
                    " where tc.Refresh_Cycle = " + lastCycle )           
